...
(`include/mta_hal.h`:223-224); `GR909` and `DOCSIS` come from the briefs
of `mta_hal_TriggerDiagnostics` (:810) and `mta_hal_GetServiceFlow` (:850).</em>
...
*Derived from `include/mta_hal.h` — the declaration set at :648-2584 and the interface overview at
...
- **The backup battery, and the twelve calls that read it.** `mta_hal_BatteryGetInstalled`
  (`include/mta_hal.h`:1666) exists precisely so that a caller can
...
*Derived from `include/mta_hal.h`:176, :585-590, :923, :956 and :1666.*
...
<em>Derived from `include/mta_hal.h`:566-572 (the lifecycle statement),
...
`include/mta_hal.h`:595-598 and :2530-2536.*
...
`include/mta_hal.h`:597-598 and the per-function thread-safety warnings,
for example :892-893.*
...
- **The return of a call is not the moment an input buffer becomes the caller's again.** This
  ...
  (`include/mta_hal.h`:1152-1177). The structure passed to
...
- Do not retain a caller-supplied pointer after the call returns. This is an obligation on the
  ...
  `mta_hal_start_provisioning()` at `include/mta_hal.h`:2489-2494 and
...
`include/mta_hal.h`:600-605 (the interface-wide statement), :767-771
(`pCalls`), :885-887 (`ppCfg`), :1606-1612 (`pDescription`), :484-1171 (the `pPINString` lifetime
statement) and :2443-2449 (the provisioning parameters).</em>
...
`include/mta_hal.h`:1636-2053 (the battery calls, all of which report and
...
*Derived from `include/mta_hal.h`:2540-2588 (the callback typedef, its
...
statement), :810-847 (`mta_hal_TriggerDiagnostics`) and :2262-2291 (`mta_hal_devResetNow`).</em>
...
`include/mta_hal.h`:117-128 (the two codes), :574-581 (the interface-wide
statement, its two exceptions and the battery consequence), :719-745
(`mta_hal_LineTableGetNumberOfEntries`) and :2584 (the registration function).</em>
...
`include/mta_hal.h`:947-949, :970-973, :1000-1002, :1188-1190, :1443-1445,
...
(`DECT_MAX_HANDSETS`), :321-389 (`MTAMGMT_MTA_CALLS`) and :403-420 (`MTAMGMT_MTA_LINETABLE_INFO`).</em>
...
`build_ut.sh`:22-33, which clones and delegates to the unit-test suite.*
...
*Derived from `include/mta_hal.h`:26-27 and :2646 (the include guard) and
...
*Derived from `include/mta_hal.h`:560-597 (the interface-wide
...
| Type | Declared at | What it represents |
| --- | --- | --- |
| `MTAMGMT_MTA_DECT` | :189-197 | The `DECT` base station's identity and versions: hardware and software version, the `RFPI` held in EEPROM, the authentication `PIN`, and the handset registration and deregistration status members. Read by `mta_hal_GetDect()`. |
| `MTAMGMT_MTA_HANDSETS_INFO` | :209-218 | One registered handset: index, status, last-active time, hardware, software and `PIN` versions, and the `OperatingTN` and `SupportedTN` telephone numbers. Returned as an array by `mta_hal_GetHandsets()`. |
| `MTAMGMT_MTA_DHCP_INFO` | :229-249 | The MTA's IPv4 lease and addressing: address, subnet mask, gateway, primary and secondary DNS and `DHCP` servers, lease, rebind and renew times, boot file name, `FQDN`, MAC address and `DHCP` options 3, 6, 7 and 8. Read by `mta_hal_GetDHCPInfo()`. |
| `MTAMGMT_MTA_DHCPv6_INFO` | :262-282 | The IPv6 equivalent, with addresses held as text of `INET6_ADDRSTRLEN` and a `Prefix` in place of a subnet mask. Read by `mta_hal_GetDHCPV6Info()`. |
| `MTAMGMT_MTA_SERVICE_FLOW` | :295-312 | One `DOCSIS` service flow with its `QoS` parameters: flow identifier, service class name, direction, schedule type, default-flow flag, grant and poll intervals, jitter tolerance, reserved and maximum rates, burst size, traffic type and packet count. Returned as an array by `mta_hal_GetServiceFlow()`. |
| `MTAMGMT_MTA_CALLS` | :321-389 | The voice-quality record of one call — the largest structure in the interface at 66 members. It carries the local and remote codec, call start and end time, duration, originator and remote address, and then the local and remote quality metrics: `MOS-LQ` and `MOS-CQ`, R-factor, signal, noise and echo return level, loss and discard rate, burst and gap density and duration, round-trip delay, jitter and jitter-buffer behaviour, and packet and octet counts. Returned as an array by `mta_hal_GetCalls()` and reachable through the `pCalls` member below. |
| `MTAMGMT_MTA_LINETABLE_INFO` | :403-420 | One voice line: instance and line number, on-hook or off-hook status, the four `GR909` loop-test results, ringer equivalency, circuit-assurance name and port, message-waiting indicator, over-current fault status, and a `pCalls` pointer to `CallsNumber` call records with the time they were last updated. Read by `mta_hal_LineTableGetEntry()`. |
| `MTAMGMT_MTA_CALLP` | :432-437 | The call-processing state of one line: line-card state, call-processing state and loop current, each as text. Read by `mta_hal_GetCALLP()`. |
...
| `RETURN_ERR` | -1 | Failure, with no cause. The interface's only failure code (:127). |
| `TRUE` / `FALSE` / `ENABLE` | 1 / 0 / 1 | Control values for the `BOOLEAN` arguments the setters take (:103, :108, :114). |
| `IPV4_ADDRESS_SIZE` | 4 | Octets in an IPv4 address, and the length of the `Dot` array in `ANSC_IPV4_ADDRESS` (:133). |
| `MTA_HAL_SHORT_VALUE_LEN` | 16 | Byte size of each short fixed-width text field of `MTAMGMT_MTA_CALLS`, terminator included (:140). |
...
*Derived from `include/mta_hal.h`:50-172, :196-495, :515-545, :2361-2422
and :2535-2584.*
...
*Derived from `include/mta_hal.h`:648 and :2528 (the bootup pair),
...
<em>Derived from `include/mta_hal.h`:508-532 (the enumeration, its five
...
and :2150-2350 (the reads that report status).</em>
